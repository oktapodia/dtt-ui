...
export default class Token extends React.Component {
  ...
  handleTokenDialog = () =>
    dialog.showOpenDialog({ properties: ['openFile'] }, fileName => {
      try {
        this.setState({ tokenFile: fileName[0] });
      } catch (e) {
        this.setState({ tokenFile: '' });
      }
    });
  ...
  handleSaveToken = () => {
    ...
    if (this.state.tokenFile !== '') {
      helper.saveToken(this.state.tokenFile).then(() => {
        this.props.check();
        this.props.close();
      });
    } else {
    ...
    return (
      <div>
        <span style={{ lineHeight: 1.5 }}>
          In order to get token, please login to the GDC Data Portal and click on "Download Token"
          from your Username dropdown
        </span>
        <button
          style={{ marginTop: 7 }}
          onClick={() => {
        ...
        <Dropzone
          ...
          onDrop={(acceptedFiles, rejectedFiles) => {
            this.setState({ tokenFile: acceptedFiles[0].path });
            acceptedFiles = [];
          }}
          multiple={false}
          disableClick
        >
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <span style={{ marginRight: 5 }}>Drag your token into here or browse for file</span>
        ...
        <div style={{ color: 'rgb(187, 54, 54)', marginTop: 10, lineHeight: 1.5 }}>
